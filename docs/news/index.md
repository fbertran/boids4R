...
- Declared `ggWebGL (>= 0.4.0)` as the optional WebGL backend now that
  it is on CRAN, and guarded examples/tests against older adapter APIs.

## boids4R 0.2.0

- Added a scenario-gallery vignette covering built-in 2D and 3D swarm
  scenarios, renderer-neutral frame summaries, species-level
  diagnostics, base graphics snapshots, and optional `ggWebGL` handoff.
...
- Tuned custom workflow examples so the solid obstacle diagnostics show
  positive final obstacle clearance while preserving soft steering
  semantics.
